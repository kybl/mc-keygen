...
impl EdwardsPoint {
    ...
    /// Like [`EdwardsPoint::compress_batch`], but only encodes the
    /// y-coordinate and leaves the sign bit zero. Skips the `X·Z⁻¹` multiply
    /// per point (one field mul saved per input). Intended for callers that
    /// only need the low bytes of the encoding — e.g. vanity-prefix scans —
    /// and that recompute the true sign (via [`EdwardsPoint::compress`]) on the
    /// rare hit. The high bit of byte 31 is always 0 in the returned encodings.
    pub fn compress_batch_y_only<const N: usize>(inputs: &[EdwardsPoint; N]) -> [[u8; 32]; N] {
        let mut zs: [_; N] = core::array::from_fn(|i| inputs[i].Z);
        FieldElement::invert_batch(&mut zs);
